# Contributing to S-EMBER

Thanks for your interest in contributing to S-EMBER. This repository is a benchmark-specific fork of lmms-eval, so most development patterns follow upstream lmms-eval while S-EMBER-specific changes live under `lmms_eval/tasks/sember/`, `scripts/`, and `tools/`.
...
- Python >= 3.10
- Git
- `pip` for editable local installs

### Setup

```bash
git clone https://github.com/facebookresearch/S-EMBER.git
cd S-EMBER
...
```bash
SEMBER_VIDEO_DIR=$PWD/data/videos python -m lmms_eval eval \
    --model dummy \
    --tasks sember_mcq \
    --batch_size 1 \
    --limit 1
...
4. **Test your benchmark**:
   ```bash
   python -m lmms_eval eval --model dummy \
...
1. Check existing repository issues for duplicates
...
Use editable installs for local S-EMBER development:

```bash
pip install -e ".[video,video-legacy]"
```

Keep dependency changes minimal because this repository tracks an upstream lmms-eval fork.

## Questions?

- **Issues**: use this repository's GitHub Issues.
- **Data and evaluation**: see `README.md` and `data/README.md`.
